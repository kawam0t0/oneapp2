...
"[project]/oneapp/app/api/notifications/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET() {
    ...
    try {
        ...
        const [rows] = await connection.execute("SELECT id, title, message, image_url, created_at, is_read FROM notifications ORDER BY created_at DESC LIMIT 50");
        return __TURBOPACK__imported__module__$5b$project$5d2f$oneapp$2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json(rows);
    } catch (error) {
...
async function POST(request) {
    ...
    try {
        ...
        connection = await getConnection();
        const jstNow = new Date().toLocaleString("sv-SE", {
            timeZone: "Asia/Tokyo"
        }).replace("T", " ");
        await connection.execute("INSERT INTO notifications (title, message, image_url, created_at, is_read) VALUES (?, ?, ?, ?, false)", [
            title,
            message,
            image_url || null,
            jstNow
...
//# sourceMappingURL=%5Broot-of-the-server%5D__faa834b9._.js.map